...
if [[ $EUID -eq 0 ]]; then
  ...
  echo "It will use sudo only for package/system installation." >&2
  exit 1
fi
...
node_ok() {
  command -v node >/dev/null 2>&1 && node - <<'NODE' >/dev/null 2>&1
...
if ! node_ok || ! command -v npm >/dev/null 2>&1 || ! python3 -m venv --help >/dev/null 2>&1; then
  echo "Installing controller-local build/runtime prerequisites from Debian..."
  sudo apt-get update
  sudo DEBIAN_FRONTEND=noninteractive apt-get install -y nodejs npm python3-venv curl
fi

node_ok || { echo "Installed Node $(node --version 2>/dev/null || echo unknown) is too old; need Node 20.19+, 22.12+, or newer." >&2; exit 2; }
command -v npm >/dev/null 2>&1 || { echo "npm is unavailable after dependency installation" >&2; exit 2; }
...
echo
if sudo grep -q 'CHANGE-ME' /etc/lghs/fleet-web.env 2>/dev/null; then
