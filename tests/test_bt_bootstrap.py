...
class BluetoothSourceInvariants(unittest.TestCase):
    ...
    def test_cloudflare_is_verified_before_fleet_token_is_minted(self):
        ...
        self.assertIn('pending-cloudflare.json', student)
        self.assertIn('Resuming Cloudflare-ready checkpoint', student)
        self.assertLess(student.index('os.replace(tmp_pending, PENDING)'), student.index('send_frame(sock, {\n            "v": PROTOCOL_VERSION, "type": "result", "status": "cloudflare-ready"'))
        self.assertIn('PENDING.unlink(missing_ok=True)', student)
        first_report = controller.index("first_report = wait_first_fleet_report(device_id")
        release_enroll = controller.index("release_report = enroll_release_verification_key(device_id)")
        consume = controller.rindex("consume_bootstrap_token(device_id)")
        self.assertLess(mint_call, first_report)
        self.assertLess(first_report, release_enroll)
        self.assertLess(release_enroll, consume)
        self.assertIn("bootstrap credential retained", controller)
        self.assertIn('"first-telemetry"', controller)
        self.assertIn('"release-signing"', controller)
        self.assertIn('entry["first_telemetry_at"]', controller)
        self.assertIn('entry["release_signing_at"]', controller)
        self.assertIn('RELEASE_SIGNING_KEY.is_file()', controller)
        self.assertIn('row.get("id") == "release.signing-key"', controller)
