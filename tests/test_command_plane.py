...
class CommandPlaneTests(unittest.TestCase):
    ...
    def test_status_uses_https_cache_as_runtime_liveness_authority(self):
        ...
        with mock.patch.object(mod,'remote_report',side_effect=AssertionError('fresh Fleet cache must not require SSH')):
            ...
            stale=dict(base);stale['received_at']=now-30;mod.load_cache=lambda:{'CS-999':stale};out=io.StringIO()
            with contextlib.redirect_stdout(out):rc=mod.fleet_status()
            self.assertEqual(rc,1);self.assertIn('STALE',out.getvalue())
            offline=dict(base);offline['received_at']=now-61;mod.load_cache=lambda:{'CS-999':offline};out=io.StringIO()
            with contextlib.redirect_stdout(out):rc=mod.fleet_status()
            self.assertEqual(rc,1);self.assertIn('OFFLINE',out.getvalue())
            rebooting=dict(offline);rebooting['lifecycle']={'state':'planned_shutdown','reason':'reboot','boot_id':'old'};mod.load_cache=lambda:{'CS-999':rebooting};out=io.StringIO()
            with contextlib.redirect_stdout(out):rc=mod.fleet_status()
            self.assertEqual(rc,0);self.assertIn('REBOOTING',out.getvalue())
            shutdown=dict(offline);shutdown['lifecycle']={'state':'planned_shutdown','reason':'poweroff','boot_id':'old'};mod.load_cache=lambda:{'CS-999':shutdown};out=io.StringIO()
            with contextlib.redirect_stdout(out):rc=mod.fleet_status()
